...
6. Which AWS service allows companies to connect an Amazon VPC to an on-premises data centre? (Select TWO)
    - A. AWS VPN.
    - B. Amazon Redshift.
    - C. API Gateway.
...
14. Which design principles for cloud architecture are recommended when rearchitecting a large monolithic application? (Select TWO)
...
22. Under the AWS shared responsibility model, which of the following activities are the customer’s responsibility? (Select TWO)
    ...
    - E. Maintaining environmental controls within a data centre.
